...
public class UserInfoController {
    ...
    public String insertUserInfo(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        try {
            ...
            Boolean TF = Boolean.parseBoolean(CmmUtil.nvl(request.getParameter("user_type")));
            int user_type = 0;
            //일반회원 /보호소 담당자 여부)
            if(TF) {
                user_type = 1;
    ...
    public String login() {
        log.info(this.getClass().getName() + ".login ok!");
    ...
    public String MyPage(HttpSession session, ModelMap model) throws Exception {
        ...
        UserInfoDTO pDTO = userInfoService.getUserInfo(uDTO);
        //이메일 디코딩
        String userEmail = EncryptUtil.decAES128CBC(pDTO.getEmail());
        pDTO.setEmail(userEmail);
    ...
    public String Updatemypage(HttpSession session, HttpServletRequest request) throws Exception {
        log.info("회원정보 업데이트 시작!");
        ...
        uDTO.setUser_id(user_id);
        userInfoService.Updatemypage(uDTO);
        log.info("마이페이지 회원명 변경!");
    ...
    public String deleteUser(HttpSession session) throws Exception {
        ...
        UserInfoDTO uDTO = new UserInfoDTO();
        uDTO.setUser_id(user_id);
        userInfoService.deleteUser(uDTO);
        log.info("마이페이지 회원 탈퇴!");

...
public class UserInfoService implements IUserInfoService {
    ...
    public UserInfoDTO getUserLoginCheck(UserInfoDTO pDTO) throws Exception {
         ...
         */
        if (CmmUtil.nvl(pDTO.getUser_id()).length() > 0) {
            res = 1;
            ...
            //아이디, 패스워드 일치하는지 체크하는 쿼리에서 이메일 F값 받아오기(아직 암호화되어 넘어오기 때문에 복호화 수행함)
            mDTO.setToMail(EncryptUtil.decAES128CBC(CmmUtil.nvl(rDTO.getEmail())));

            mDTO.setTitle("로그인 알림!"); //제목

            //메일 내용에 가입자 이름넣어서 내용 발송
            mDTO.setContents(DateUtil.getDateTime("yyyy.MM.dd 24h:mm:ss") + "YourNeulBom ID => " + CmmUtil.nvl(rDTO.getUser_id()) + "is Online");
    ...
    public int forgetPassword(UserInfoDTO pDTO) throws Exception {
        ...
        if (CmmUtil.nvl(rDTO.getUser_id()).length() > 0) {
            ...
            mDTO.setTitle("늘봄 회원 비밀번호 변경입니다."); //제목

            String url = "52.79.103.219:11000/resetPassword?id=" + pDTO.getUser_id();
            //메일 내용에 가입자 비밀번호 넣어서 내용 발송
            mDTO.setContents(DateUtil.getDateTime("yyyy.MM.dd 24h:mm:ss") + "Dear," + CmmUtil.nvl(rDTO.getUser_name()) + " Go to URL and Reset Password!" + url);
    ...
    @Override
    public void Updatemypage(UserInfoDTO pDTO) throws Exception {

        log.info(this.getClass().getName() + ".Updatemypage start!");

        userInfoMapper.Updatemypage(pDTO);
    }

    @Override
    public void deleteUser(UserInfoDTO dDTO) throws Exception {

        log.info(this.getClass().getName() + ".deleteUser start!");

        userInfoMapper.deleteUser(dDTO);
    }
}
